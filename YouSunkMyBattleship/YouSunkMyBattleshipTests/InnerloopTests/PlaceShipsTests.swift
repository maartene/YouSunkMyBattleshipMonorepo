...
@Suite(.tags(.`Unit tests`)) struct PlaceShipsTests {
    ...
    @Suite struct `Interaction between View and ViewModel` {
        ...
        @Test func `when a drag starts, the viewmodel is notified`() throws {
            // Send gesture
            let value = DragGesture.Value(time: Date(), location: CGPoint(x: 70, y: 300), startLocation: CGPoint(x: 70, y: 300), velocity: .zero)
        ...
        @Test func `when a drag ends, the viewmodel is notified`() throws {
            let value = DragGesture.Value(time: Date(), location: CGPoint(x: 200, y: 300), startLocation: CGPoint(x: 200, y: 300), velocity: .zero)
    ...
    @Suite struct ViewModelTests {
        ...
        @Test func `given a drag already started, when a drag moves to  195,370 then the cells A5, B5 and C5 becomes a ship`() {
            viewModel.startDrag(at: CGPoint(x: 195, y: 301))
        ...
        @Test func `given a drag already started, when a end at 195,370 then the cells A5, B5 and C5 becomes a ship`() {
            viewModel.startDrag(at: CGPoint(x: 195, y: 301))
        ...
        @Test func `given a drag already started, when a drag is outside of an ocean tile, the cells remain ships`() {
            viewModel.startDrag(at: CGPoint(x: 195, y: 301))
            viewModel.startDrag(at: CGPoint(x: 195, y: 370))
        ...
        @Test func `given a drag already started, when a drag ends outside of the board, the drag is reset`() {
            viewModel.startDrag(at: CGPoint(x: 195, y: 370))
        ...
        @Test func `when all ships have been placed, the viewmodel should signal to confirm placement`() {
            completePlacement(on: viewModel)
            
            #expect(viewModel.state == .awaitingConfirmation)
        }
        
        @Test func `given all ships have been placed, when the player confirms placement, the viewmodels state should move to play`() async {
            completePlacement(on: viewModel)
        ...
        @Test func `given all ships have been placed, when the player confirms placement, the game services setBoard should be called`() async {
        ...
        @Test func `given all ships have been placed, when the player cancels placement, the board is reset`() {
            completePlacement(on: viewModel)
        ...
        @Test func `given all ships have been placed, when the player confirms placement and an error is occurred, the state does not change`() async throws {
    ...
    @Suite struct GameLogicTests {
        ...
        ]) func `when a valid ship has been placed, it is removed from the ships to place list`(testcase: (coordinates: [Coordinate], expectedShip: Ship)) {
            var board = Board()
        ...
        @Test func `when a larger ship tries to be placed than is available, it does not get placed`() {
            var board = Board()
            let coordinates = [
                Coordinate(x: 1, y: 1),
                Coordinate(x: 2, y: 1),
        ...
        @Test func `when a ship is placed where another ship has already been placed, it does not get placed`() {
            var board = Board()
            let coordinates = [
                Coordinate(x: 1, y: 1),
                Coordinate(x: 1, y: 2),
