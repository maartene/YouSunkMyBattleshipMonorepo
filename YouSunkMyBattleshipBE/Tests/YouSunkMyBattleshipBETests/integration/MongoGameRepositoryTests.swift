...
@Suite struct MongoGameRepositoryTests {
    ...
    @Test func `should be able to get a list of games`() async throws {
        let games = (0 ..< 10)
            .map { i in
                Game(gameID: "game\(i)", player1Board: .makeAnotherFilledBoard(), player2Board: .makeFilledBoard())
            }
...
extension Game: @retroactive Equatable {
    public static func == (lhs: Game, rhs: Game) -> Bool {
        lhs.player1Board == rhs.player1Board &&
        lhs.player2Board == rhs.player2Board &&
        lhs.currentPlayer == rhs.currentPlayer &&
        lhs.shipsToDestroy == rhs.shipsToDestroy &&
        lhs.state == rhs.state
    }
